...
MODULE_PARM_DESC(mode, "0:mode0; 1:mode1; 2:mode2;  used as insmod edu_driver.ko mode=0");
...
MODULE_DEVICE_TABLE(pci, pci_ids); //热插拔设备
...
static irqreturn_t irq_handler(int irq, void *dev)
{
	...
	devi = *(int *)dev;
	if (devi == major) {//比较硬件信息，如果是就处理中断，不是立马返回IRQ_NONE
...
static int pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	/* __pci_request_region 中判断是PIO还是MMIO */
	if (pci_request_region(dev, BAR, "myregion0")) {
		dev_err(&(pdev->dev), "pci_request_region error\n");
		goto error;
	}
	...
	/* enable MSI */
    	int nr_vectors, ret, nvec;
    	nvec = pci_msi_vec_count(pdev);
    	dev_info(&pdev->dev, "----%d", nvec);
    	nr_vectors = pci_alloc_irq_vectors(pdev, 1, 1, PCI_IRQ_MSI);
    	//nr_vectors = pci_alloc_irq_vectors(pdev, 1, 1, PCI_IRQ_ALL_TYPES);
    	dev_info(&(dev->dev), "INFO: Request for %d msix vectors\n", nr_vectors);
    	if (nr_vectors < 0) {
    	    dev_err(&(dev->dev), "Request for %d msix vectors failed\n", nr_vectors);
			goto error;
    	}

		ret = request_irq(pci_irq_vector(pdev, 0), irq_handler, 0, "wlm_msix", &major);
		if (ret < 0) {
			dev_err(&(dev->dev), "request_irq\n");
			goto error;
		}
...
static void pci_remove(struct pci_dev *dev)
{
	pr_info("pci_remove\n");
	if (mode == 1) {
		dev_info(&dev->dev, "remove the irq_vectors");
