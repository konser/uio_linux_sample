/*
 * system_wq 是个驱动公用的队列。是linux预先设置好的一个使用频率hen很高的队列
 * 并且封装了schedule_work函数来嗲用
 * 加载驱动模块之后，如果监测到键盘上的ESC键被按下，那么就往内核默认的工作队
 * 列system_wq中增加一个工作项，然后观察该工作项对应的处理函数是否被调用。
 * $ make
 * $ sudo insmod sys_wq.ko irq=1 devname=mydev
 */
