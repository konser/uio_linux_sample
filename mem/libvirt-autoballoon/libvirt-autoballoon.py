...
class LibVirtAutoBalloon:
    ...
    def dom_print_names(self):
        ...
        for i in self.conn.listAllDomains():
            domainNames += [i.name()]
        print("Found domains:", domainNames)
    ...
    def dom_usable_ratio(self, dom):
        usable = dom.memoryStats().get("unused", 0)
        return float(usable) / self.dom_keep_usable(dom)
...
def dom_ram_used(dom):
    memstat = dom.memoryStats()
    return memstat.get("actual", 0) - memstat.get("unused", 0)
...
def dom_balloon(dom, restrict_to):
    ...
    if actual > restrict_to:
        print("Shrink dom:",
              name,
              actual_m, "-", diff, "=", restrict_to_m, "MiB")
    else:
        print("Grow dom:",
              name,
              actual_m, "+", diff, "=", restrict_to_m, "MiB")
...
def help():
    print("Usage: libvirt-autoballoon <arg>")
    print("    start  - start daemon")
    print("    status - show what daemon see")
    exit(0)
