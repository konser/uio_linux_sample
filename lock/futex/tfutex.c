/* 
 * sample show how to use futex betwen two process, and use system v  
 * shared memory to store data
 * gcc -lrt -o tf tfutex.c 
 *
 * systemcall NO. is 240
 *  #define __NR_futex              240
 * https://cloud.tencent.com/developer/article/1176832
 * FUTEX_WAIT和FUTEX_WAKE只是用来挂起或者唤醒进程
 *
 */  
...
{  
    ...
    if (fd < 0)  
    {  
        printf("shm_open failed, errormsg=%s errno=%d\n", strerror(errno), errno);  
        return 0;  
    }  
    ftruncate(fd, size);  
    ...
    if (pid == 0) { // child process  
        ...
        *ptr = 3;  
        ...
        struct namelist *cur = (struct namelist *)(ptr+1);  
