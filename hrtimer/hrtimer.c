...
static unsigned int interval=1; /* unit: s */
...
{ 
    ...
    /* 
     * return the sum(secs + nsecs)
     * ktime_t ktime_set(const long secs, const unsigned long nsecs) 
     * 
     * 0 + 5000000ns = 0s + 5ms = 5ms
     * */
    //ktime = ktime_set(interval/1000000, (interval%1000000)*1000);
    /* 1s */
    ktime = ktime_set(interval, 0);
    /*
     * int hrtimer_start(struct hrtimer * timer, ktime_t tim, const enum hrtimer_mode mode);
     *
     * tme: expiry time
     * mode: 
     * absolute (HRTIMER_MODE_ABS) 
     * relative (HRTIMER_MODE_REL)
     * cpu bind (HRTIMER_MODE_PINNED)
     *
     */
    ...
    if(count%COUNT_INTERVAL==0) 
    {
        ...
        ktime_get_ts(&uptime);
        printk(KERN_INFO"ktime:%9lu ns, hrtimer:%9lu sec, %9lu ns, interval_delay=%lu ns\n",
            (unsigned long) ktime, 
...
static int __init module_hrtimer_init(void)
{
     ...
     * -> create_workqueuee("my_queue")
     * -> INIT_WORK
     * -> queue_work(p_queue, &my_work);
     *  can create self-queue instead of shared-queue 
     *
